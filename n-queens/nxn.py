...
def nxn(n):
    ...
    while mySolver.check() == sat:
        ...
        # if the solution is satisfiable then its model (list of rows (i) and
        # corresponding spots in the column  where the queen should be placed
        # (j)) will be printed along with a list of column (j) values for
        # each row (i)
        if display_sol:
            print(model)
            print('\n')
            ...
            # this loop builds a numpy matrix of the first solution to be
            # printed after
            for val in solution:
                val = str(val)
                val = int(val) - 1
            ...
            # this loop iterates over a list and adds a Or statement for the
            # next solution same for lines 85-87
            for i in range(n):
