...
std::shared_ptr<Actor> SpartyFactory::Create(std::wstring imagesDir)
{
    ...
    auto flag = std::make_shared<ImageDrawable>(L"MSU Flag", imagesDir + L"/msu_flag.png");
    flag->SetCenter(wxPoint(89, 20));
    flag->SetPosition(wxPoint(20 - 169, 22 - 300));
    torso->AddChild(flag);
    ...
    headt->SetCenter(wxPoint(50, 138));
