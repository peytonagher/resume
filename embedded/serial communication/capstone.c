...
typedef struct
{
  ...
  uint8_t dataLength;                         // number of data bytes (<=64)
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
  ...
  if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
  {
      ...
      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0,1); // green on-board LED
      ...
      // Changes the variable flag messageRecieved to 1 to indicate message has been recieved
      messageRecieved = 1;
  }
...
int main(void)
{
  ...
  txHeader1.Identifier = 0x11;
    ...
    while (1)
    {
    	 // Checks to see if the message is received in the RxFifo0 Callback function
    	if (messageRecieved)
    			{
    					messageRecieved = 0;
    	            	continue;
    	...
    	// Checks to see whether the button on the external circuit is pushed or not.
        GPIO_PinState pinStatus = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_3);

        // Conditional to see if the button is not pushed
        if (pinStatus == GPIO_PIN_RESET )
        {
        	// Switches off the Blue LED on the external circuit
            HAL_GPIO_WritePin(GPIOG, GPIO_PIN_2, 0);
            // Toggles the Red onboard LED
            HAL_GPIO_TogglePin(GPIOG, GPIO_PIN_4);
            // Checks to see if the Green onboard LED is switched on
        ...
        // Conditional to see if the button is pushed
        else if(pinStatus == GPIO_PIN_SET )
        {
        	// Switches on the Blue LED on the external circuit
            HAL_GPIO_WritePin(GPIOG, GPIO_PIN_2, 1);

            // Increments the message id by 1
            msg8.id += 1;
            // Increments the transmitter identifier by 1
            txHeader1.Identifier += 1;
            // Writes the 8 bit data to the variable msg8.
            for (int i = 0; i < msg8.dataLength; i++)
            {
                msg8.data[i] += 0x01;
            }
            ...
            printf("\nID: %lX\n", msg8.id);
            // Checks to see if there was an error with the function FDCAN_SendMessage
        	if (FDCAN_SendMessage(&msg8)!= HAL_OK)
        	{
        		...
        		// If the FDCAN_SendMessage function is okay, prints a message
        		printf("Msg sent and received \n");
        	}
        }
    }
